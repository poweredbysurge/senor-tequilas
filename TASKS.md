...
| D9 | Game Day has no desktop nav entry. It is in the mobile menu, entry 74, but the header nav is full: it appears from 1200 up and has 0px spare at 1200, 20px at 1280 and 106px at 1366, against roughly 86px for a ninth item, so adding one overflows between 1200 and about 1340. Options measured: swap the "Gift Cards" item, which is the only nav entry that is an in-page anchor rather than a page; or shorten two labels to free the width; or raise the desktop-nav breakpoint to 1366 and give 1200 to 1365 the hamburger. Needs a call on which. | open | 74 |
